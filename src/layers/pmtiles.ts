...
export async function addPMTilesLayers(
  ...
  for (let i = 0; i < layers.length; i++) {
    ...
    try {
      ...
      const helperLayerNames = new Set<string>([
        // We've seen PMTiles generated by some pipelines include an extra layer (e.g. `inputx`)
        // that contains "helper" geometries (often points at tile corners).
        // Auto-mode should avoid rendering these by default.
        'inputx',
      ]);

      const autoAll = !requestedSourceLayer && meta.allLayerNames.length > 1;
      const autoAllFiltered = autoAll
        ? meta.allLayerNames.filter((n) => !helperLayerNames.has(n))
        : meta.allLayerNames;
      ...
      if (sourceLayerNamesToRender.length === 0) {
        ...
        if (excluded.length > 0) {
          console.log(`[FusedMaps] PMTiles: auto excluded helper source-layers for ${layer.id}:`, excluded);
        }
      } else {
        console.log(
